...
async fn init(
    ...
    let mt_contract =
        worker.dev_deploy(include_bytes!("../res/multi_token.wasm").to_vec()).await?;
...
async fn test_total_supply() -> anyhow::Result<()> {
    ...
    let (contract, _, _, _) = init(&worker, initial_balance).await?;

    let res = contract.call(&worker, "mt_total_supply").view().await?;
